...
Instead of thinking in `r length(time_points)` dimensions, there is also an _integrated_ Brier score. This required evaluation times as inputs but instead of returning each result, it takes the area under the curve shown above. The syntax is the same but the result has a single row: 
...
time_as_binary_event <- function(surv, eval_time) {
  disposition_surv <- .extract_surv_time(surv)
  status <- .extract_surv_status(surv)
  is_event_before_t <- disposition_surv <= eval_time & status == 1
  ...
  binary_res <- rep(NA_character_, length(disposition_surv))
  ...
  binary_res <- if_else(disposition_surv > eval_time, "non-event", binary_res)
...
pred_early <- augment(survreg_fit, complaints_val, eval_time = 10)
...
More probability values are to the right of the 50% cutoff for the true non-events. Conversely, true events tend to have smaller probabilities. Using this cutoff, the sensitivity would be `r round(stats_early$.estimate[2] * 100, 1)`% and the specificity would be `r round(stats_early$.estimate[3] * 100, 1)`%. There are other possible cutoffs for the survival probabilities. Maybe one of these would have better statistics. 
...
Below is a contrived (but common) case with two classes. The probability distributions between the true classes are shown on the left. Note that the curves show separation between the event and non-event groups. As we would expect, the area under the ROC curve is very high (`r signif(roc_val, 3)`).

The problem is that the predicted probabilities are not realistic. They are too close to the commonly used cutoff of 0.5. Across all of the data, the probabilities only range from `r round(min(example_data$.pred_event), 2)` to `r round(max(example_data$.pred_event), 2)`. We would expect these to be much closer to zero and one, respectively.
...
if (!file.exists("auc_plot_data.RData")) {
  ...
  get_roc_curves <- function(time) {
    pred_vals <- augment(survreg_fit, complaints_val, eval_time = time)
    pred_vals %>%
...
if (!file.exists("cal_plot_data")) {
  ...
  obs_rate <- function(ind, mid, truth, wts) {
    ...
    tibble(expected = mid, observed = res$estimate,
...
binary_encoding %>% 
  ...
  lims(x = 0:1) +
  labs(x = "probability of event at or before eval time") +
  theme_bw()
```

The range of probabilities at 10 days is almost the entire range, and there is moderate separation between the two. However, at 100 days, the smallest probability prediction is `r round(small_100, 2)`. Clearly, these predictions are not well calibrated. The calibration plots tell the same story:
...
It may be difficult to tell from the histograms above, but the area under the ROC curve at 100 days is `r round(auc_100, 2)`. That's not bad; the metric is seeing separation despite the lack of accuracy. 
