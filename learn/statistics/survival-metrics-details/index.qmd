...
First, let's dive into how to convert the observed event time in `disposition_surv` to a binary version. Then we will discuss the remaining columns as part of generating the required weights for the dynamic performance metrics.
...
dyn_val_pred %>% 
  ...
  ylim(0:1) + 
  geom_rug(
    data = complaints_train %>% filter(disposition_surv[,2] == 1), 
    aes(x = disposition_surv[,1]), 
    alpha = 1 / 2, col = "red"
  ) + 
  geom_rug(
    data = complaints_train %>% filter(disposition_surv[,2] == 0), 
    aes(x = disposition_surv[,1]), 
...
We call this time at which to predict the probability of censoring the _weight time_. 
...
dyn_val_pred %>% 
  filter(.row == 29 & .eval_time <= 35) %>% 
  select(disposition_surv, .eval_time, .weight_time, .pred_censored, .weight_censored)
```

```{r}
#| label: example-time-hide
#| include: false
time_1 <- parsnip::.extract_surv_time(complaints_val$disposition_surv[1])
```
...
#### An censored time {.unnumbered}

Now let's look at a censored data point: 
...
dyn_val_pred %>% 
  filter(.row == 1 & .eval_time <= 40) %>% 
  select(disposition_surv, .eval_time, .weight_time, .pred_censored, .weight_censored)
```

#### Even finer details {.unnumbered}
...
dyn_val_pred %>% 
  ...
  lims(y = c(0, nrow(complaints_val))) +
  theme_bw()
```

Keeping this in mind, let's look at what happens with the data points we can use. Let's start with an evaluation time of 30. To compute the confusion matrix for a classification problem, we would simply use:
...
#| include: false 
dat_early <- binary_encoding %>% filter(.eval_time == 30 & !is.na(.weight_censored))
events_early <- nrow(dat_early %>% filter(disposition_surv[,1] <= 30))
```

The values in the cells are the sum of the censoring weights, There are `r events_early` actual events (out of `r nrow(dat_early)` usable observations) before this evaluation time, so there are empty cells. Also note that the cell values are close to the actual counts. This early, the predicted censoring probabilities are very close to one so their inverse values are also. 
...
```{r}
#| label: conf-mat-mid-hide
#| include: false 
dat_mid <- binary_encoding %>% filter(.eval_time == 90 & !is.na(.weight_censored))
events_mid <- nrow(dat_mid %>% filter(disposition_surv[,1] <= 90))
...
Now we have fewer total observations to consider (`r nrow(dat_mid)` instead of `r nrow(dat_early)` usable values) and more events (`r events_mid` this time). Performance is fairly good; the sensitivity is `r round(stats_mid$.estimate[2] * 100, 1)`% and the specificity is `r round(stats_mid$.estimate[3] * 100, 1)`%.

What happens when the evaluation time is 17?
...
```{r}
#| label: conf-mat-late-hide
#| include: false 
dat_late <- binary_encoding %>% filter(.eval_time == 90 & !is.na(.weight_censored))
events_late <- nrow(dat_late %>% filter(disposition_surv[,1] <= 90))
stats_late <- 
  binary_encoding %>%
  filter(.eval_time == 90) %>%
  cls_set(truth = obs_class,
...
The data are overwhelmingly events. Also, the censoring weights are larger now since the probability of being censored is very low. The mean censoring weight is `r round(mean(dat_late$.weight_censored), 2)`.
