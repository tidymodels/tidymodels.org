...
TODO: Look at the amount of censoring?
...
Let's take a look at the AUC ROC first.
...
The accuracy of the predicted probabilities is generally good, albeit lowest for evaluation times of 30 and 60 days. The integrated Brier score is a measure of the overall accuracy of the predicted probabilities. Since we don't have a particular evaluation time that we want to predict well at, we are going to use the integrated Brier score as our main performance metric.
...
For the random forest, we are using the `"aorsf"` engine for accelerated oblique random survival forests. Oblique trees are a type of tree that can split on linear combinations of the predictors, i.e., they provide more flexibility in the splits than trees which split on a single predictor.
...
The best regularized Cox model performs a little better than the parametric survival model, with an integrated Brier score of 0.0481968 compared to 0.0508158 for the parametric model. The random forest yet a little better with an integrated Brier score of 0.046217.
