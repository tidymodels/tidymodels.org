...
A better solution is _conditional execution_. In this case, we determine the unique set of candidate values associated with the preprocessor and loop over these. Within each loop, we train and predict the random forest model across its candidate values. Here's a listing of this algorithm: 
...
In general, conditional execution is a good idea. Even when the second grid is used (as with `umap_rf_grid_other` above), there is no computational loss incurred by conditional execution. This is also true if the preprocessing technique is inexpensive. We will see issues with conditional execution that comes up in @sec-grid-types as well as in @sec-in-parallel when we can run the computations in parallel. 
...
When tuning models, a single holdout set (a.k.a. a validation set) or multiple holdout sets are used (via resampling) to accurately estimate performance. For example, with 10-fold cross-validation, there is _another_ loop over the $B = 10$ resamples. 
...
```{r}
#| label: umap-boost-post-submodels
...
```{r}
#| label: umap-boost-post-thresholds
