...
```{r}
#| label: tidy-mixed-effects-example
#| eval: false
model <- mixed_model(...)
...
```{r}
#| label: extract-fitted-residuals
...
```{r}
#| label: document-tidy-lm-roxygen
#| eval: false
#' Tidy a(n) lm object
#'
#' @param x A `lm` object.
...
```{r}
#| label: download-glossary-data
#| include: false
# grab the argument glossary: make a temporary file, write the
# data to it, load it, and then delete it
args_url <- "https://raw.githubusercontent.com/alexpghayes/modeltests/main/data/argument_glossary.rda"
args_file <- tempfile()
download.file(args_url, args_file)
load(args_file)
unlink(args_file)

# do the same thing for the columns
cols_url <- "https://raw.githubusercontent.com/alexpghayes/modeltests/main/data/column_glossary.rda"
cols_file <- tempfile()
download.file(cols_url, cols_file)
load(cols_file)
unlink(cols_file)
```
...
```{r}
#| label: render-argument-glossary
#| echo: false
argument_glossary %>%
