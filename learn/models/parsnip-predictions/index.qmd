...
`r article_req_pkgs(pkgs)` There are numerous other "engine" packages that are required. If you use a model that is missing one or more installed packages, parsnip will prompt you to install them.
...
To use [Apache Spark](https://spark.apache.org/) as an engine, we will first install Spark and then need a connection to a cluster. For this article, we will setup and use a single-node Spark cluster running on a laptop.
...
The tidymodels [agua](https://agua.tidymodels.org/) package has some helpers and will also need to be installed. You can use its function to start a server too:
